...
function removeFirstOccurrences(str, value) {
  if (str.indexOf(value) !== -1) {
    const sub = str.indexOf(value);
    return str.slice(0, sub) + str.slice(sub + value.length);
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  if (str.indexOf(value) !== -1) {
    const sub = str.lastIndexOf(value);
    return str.slice(0, sub) + str.slice(sub + value.length);
  }
  return str;
}
...
function formatTime(minutes, seconds) {
  ...
  const formSeconds = String(seconds).padStart(2, '0');
...
function countVowels(str) {
  ...
  const arr = str.toLowerCase().match(/[aeiouy]/g);
...
function isPalindrome(str) {
  const lowerStr = str.replace(/[^a-z0-9]/gi, '').toLowerCase();
